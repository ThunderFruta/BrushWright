"""Minimal Paint Transformer model imported for synthetic data generation.

Adapted from https://github.com/Huage001/PaintTransformer under Apache-2.0.
Only the inference-time Painter network is kept here; training/demo code is
...
class SignWithSigmoidGrad(torch.autograd.Function):
    ...
    def backward(ctx, grad_result):
        ...
        if ctx.needs_input_grad[0]:
            return grad_result * sigmoid_result * (1 - sigmoid_result)
        return None


class Painter(nn.Module):
    def __init__(self, param_per_stroke: int, total_strokes: int, hidden_dim: int, n_heads: int = 8,
                 n_enc_layers: int = 3, n_dec_layers: int = 3) -> None:
        super().__init__()
    ...
    def forward(self, img, canvas):
        ...
        pos_embed = torch.cat(
            [
                self.col_embed[:width].unsqueeze(0).contiguous().repeat(height, 1, 1),
                self.row_embed[:height].unsqueeze(1).contiguous().repeat(1, width, 1),
            ],
            dim=-1,
        ).flatten(0, 1).unsqueeze(1)
        hidden_state = self.transformer(
            pos_embed + feat_conv.flatten(2).permute(2, 0, 1).contiguous(),
            self.query_pos.unsqueeze(1).contiguous().repeat(1, batch_size, 1),
        )
