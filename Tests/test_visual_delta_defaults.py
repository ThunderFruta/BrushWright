...
class VisualDeltaDefaultsTest(unittest.TestCase):
    ...
    def test_visual_delta_training_defaults_use_dense_proposals(self) -> None:
        ...
        self.assertTrue(config.epoch_sample_report)
        self.assertEqual(config.epoch_sample_report_train_count, 2)
        self.assertEqual(config.epoch_sample_report_val_count, 2)
        self.assertEqual(config.epoch_sample_report_interval, 1)
